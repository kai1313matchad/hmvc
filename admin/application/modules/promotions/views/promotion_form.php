              ...
              <div id="myTabContent" class="tab-content">
                <div role="tabpanel" class="tab-pane fade <?php if($this->uri->segment(2) == 'new') {?>active in<?php }?>" id="tab_content1" aria-labelledby="home-tab">
                  <form id="form-product" class="form-horizontal form-label-left" enctype="multipart/form-data">
                        ...
                        <select class="form-control" id="related_product" multiple="multiple">
                          <?php foreach ($read_product as $key => $value) {?>
                            <option value="<?php echo $value->PROD_ID ?>" 
                                <?php if($this->uri->segment(2) == 'edit') {?>
                                    <?php foreach ($read_promotion_detail as $kd => $vd) {?>
                                        <?php if($vd->PROD_ID == $value->PROD_ID){?>selected<?php }?>
                                    <?php }?>
                                <?php }?>
                            >
                    ...
                    <!-- <div class="form-group">
                      <label for="middle-name" class="control-label col-md-3 col-sm-3 col-xs-12">Background</label>
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input class="form-control col-md-7 col-xs-12 inp_background" type="color" name="background" required="required" placeholder="Background Color" value="#ffffff">
                        <span class="help-block"></span>
                      </div>
                    </div> -->
                    <div class="form-group">
                    ...
                    <div class="text-right">
                      <button type="button" class="btn btn-primary" 
                        <?php if($this->uri->segment(2) == 'new') {?>onclick="Promotion.create()"<?php } else {?>onclick="Promotion.update(this, '<?php echo $read_promotion['ID']?>')"<?php }?>>Next Step</button>
                    </div>
                  </form>
                </div>
...
<style>
  ...
  #related_product, .select2-container {
      width: 100%!important
  }
</style>
<!-- /page content -->